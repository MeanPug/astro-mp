...
(front-end URL, CORS origins, Netlify build hook; see section 10) and `astro_indexable`
("Allow search engines"): off by default, which sends `noindex, nofollow` on every page and a
blocking `robots.txt`, so test deploys never compete with the live site; turn it on for the
production domain only, and `robots.txt` then announces `sitemap-index.xml` (generated by
`@astrojs/sitemap`). Tab **Integrations**: `apexchat_company` (ApexChat company slug: chat
widget, video greeting and invitation popup, all configured in the ApexChat account),
`gtm_id` (Google Tag Manager container; pixels and analytics live inside the container) and
`callrail_script_url` (the `swap.js` URL from the CallRail snippet: number swapping and form
tracking). Empty fields disable the integration. Saving Theme Settings triggers a rebuild. Menu location `footer-1` holds the footer links. The site logo is the WordPress custom logo
